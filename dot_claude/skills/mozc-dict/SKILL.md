...
`uv run ~/.claude/skills/mozc-dict/mozc_dict.py` を使って Mozc のユーザー辞書を操作する。
...
```bash
uv run ~/.claude/skills/mozc-dict/mozc_dict.py add <読み> <単語> [品詞] --reload
```

例:
```bash
uv run ~/.claude/skills/mozc-dict/mozc_dict.py add ていね 丁寧 名詞 --reload
uv run ~/.claude/skills/mozc-dict/mozc_dict.py add かいしゃめい 株式会社〇〇 固有名詞 --reload
```

### 登録済み単語を一覧表示する

```bash
uv run ~/.claude/skills/mozc-dict/mozc_dict.py list
```

### 単語を削除する

```bash
uv run ~/.claude/skills/mozc-dict/mozc_dict.py delete <読み> <単語> --reload
```

### TSV から一括インポートする

```bash
uv run ~/.claude/skills/mozc-dict/mozc_dict.py import-tsv <ファイル> --reload
```
...
```bash
uv run ~/.claude/skills/mozc-dict/mozc_dict.py export-tsv [ファイルパス]
```
